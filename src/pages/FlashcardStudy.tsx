...
import { ArrowLeft, RotateCcw, ChevronLeft, ChevronRight, Layers, Tag, Loader2, Frown, Meh, Smile, Target, Star } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Progress } from '@/components/ui/progress';
import Navbar from '@/components/Navbar';
import Breadcrumb from '@/components/Breadcrumb';
import { getDueFlashcardReviews, getDeck, submitFlashcardReview } from '@/services/flashcards';
...
const FlashcardStudy = () => {
  ...
  const rateCard = async (quality: number) => {
    if (!card) return;
    try {
      // Async submit to background
      submitFlashcardReview(card.id, quality).catch(err => console.error(err));
    ...
    return (
      <div className="min-h-screen bg-background bg-grid">
        ...
        <div className="container max-w-2xl pt-24 px-4 pb-8 flex flex-col items-center justify-center">
          ...
          <h2 className="text-3xl font-mono font-bold mb-4">Session Complete!</h2>
          <p className="text-muted-foreground mb-8 text-center max-w-sm">
            You've reviewed {reviewedIds.size} cards. Great job building your knowledge.
          </p>
          <div className="flex gap-4">
            <Button variant="outline" className="font-mono gap-2" onClick={() => navigate(`/decks/${deckId}`)}>
              <ArrowLeft className="h-4 w-4" /> Back to Deck
            </Button>
            <Button className="glow-cyan font-mono gap-2" onClick={() => {
              setCurrentIndex(0);
    ...
    <div className="min-h-screen bg-background bg-grid">
      ...
      <div className="container max-w-2xl pt-20 px-4 pb-8">
        ...
        <div className="flex items-center justify-between mt-4 mb-6">
          <Button variant="ghost" size="sm" onClick={() => navigate(`/decks/${deckId}`)}>
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between text-xs font-mono text-muted-foreground mb-1.5">
            <span>Card {Math.min(currentIndex + 1, pool.length)} / {pool.length}</span>
            ...
            <AnimatePresence mode="wait" custom={direction}>
              <motion.div
                ...
                className={`relative glass-card p-8 min-h-[320px] md:min-h-[400px] flex flex-col justify-center cursor-pointer select-none transition-shadow ${
                  reviewedIds.has(card.id) ? 'border-primary/40 shadow-[0_0_20px_hsl(var(--primary)/0.1)]' : ''
                }`}
              >
                {card.isStarred && (
                ...
                {!isFlipped ? (
                  <div className="text-center w-full">
                    ...
                    <p className="text-muted-foreground text-xs mt-8 font-mono inline-flex items-center bg-secondary/50 px-3 py-1.5 rounded-full">
                      <Layers className="h-3.5 w-3.5 mr-2" /> Tap to reveal answer
                    </p>
                  </div>
                ) : (
                  <div className="text-center w-full flex flex-col items-center">
                    ...
                    {card.hint && (
                      <div className="mt-6 text-sm text-muted-foreground bg-secondary/50 p-3 rounded-lg w-full text-left">
                        <span className="font-mono text-xs font-bold block mb-1">HINT / NOTES</span>
                        {card.hint}
                      </div>
                    )}
                    {card.tags?.length > 0 && (
        ...
        <div className="flex items-center justify-between">
          <Button variant="outline" size="icon" onClick={goPrev} disabled={currentIndex === 0} className="h-10 w-10 shrink-0">
            <ChevronLeft className="h-5 w-5" />
          </Button>

          {isFlipped && card ? (
            <div className="flex flex-wrap items-center justify-center gap-2 mx-4">
              <Button size="sm" variant="outline" className="border-destructive/30 text-destructive hover:bg-destructive hover:text-destructive-foreground font-mono text-xs px-2 md:px-4" onClick={() => rateCard(1)}>
                <Frown className="h-3.5 w-3.5 mr-1" /> Again
              </Button>
              <Button size="sm" variant="outline" className="border-warning/30 text-warning hover:bg-warning hover:text-warning-foreground font-mono text-xs px-2 md:px-4" onClick={() => rateCard(3)}>
                <Meh className="h-3.5 w-3.5 mr-1" /> Hard
              </Button>
              <Button size="sm" variant="outline" className="border-primary/30 text-primary hover:bg-primary hover:text-primary-foreground font-mono text-xs px-2 md:px-4" onClick={() => rateCard(4)}>
                <Smile className="h-3.5 w-3.5 mr-1" /> Good
              </Button>
              <Button size="sm" variant="default" className="bg-primary hover:bg-primary/90 text-primary-foreground font-mono text-xs glow-cyan px-2 md:px-4" onClick={() => rateCard(5)}>
                <Target className="h-3.5 w-3.5 mr-1" /> Easy
              </Button>
            </div>
          ) : (
            <Button size="sm" variant="outline" onClick={() => setIsFlipped(true)}>
              <Layers className="h-4 w-4 mr-1.5" /> Reveal Answer
            </Button>
          )}

          <Button variant="outline" size="icon" onClick={goNext} disabled={isFinished} className="h-10 w-10 shrink-0">
