...
const TrapQuestionsPage = () => {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteQuery({
      ...
      queryFn: ({ pageParam = 1 }) =>
        getQuestions(certId || undefined, pageParam, 12, true),
      initialPageParam: 1,
  ...
  const certMap = new Map(certifications?.map((c) => [c.id, c.code]) ?? []);
          ...
          <div className="mb-8 flex flex-col md:flex-row gap-4 items-start justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">
                High-difficulty, tricky questions designed to expose common
                misconceptions. Practice these to avoid exam surprises.
              </p>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 border border-border rounded-xl bg-muted/30">
                ...
                <h3 className="text-lg font-semibold">
                  No trap questions found
                </h3>
                <p className="text-muted-foreground mt-1">
                  No questions have been marked as tricky yet for this
                  certification.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex gap-2 mb-3 flex-wrap">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-muted text-muted-foreground text-xs font-mono">
                        {certMap.get(q.certificationId) ?? q.certificationId}
                      </span>
                      {q.isScenario && (
