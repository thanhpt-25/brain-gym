...
export class TagsService {
    ...
    async remove(id: string) {
        const questionCount = await this.prisma.questionTag.count({ where: { tagId: id } });
        if (questionCount > 0) {
            throw new BadRequestException(`Cannot delete tag used by ${questionCount} questions`);
        }
    ...
    async merge(sourceIds: string[], targetId: string) {
        ...
        for (const sourceId of sourceIds) {
            const links = await this.prisma.questionTag.findMany({ where: { tagId: sourceId } });
            for (const link of links) {
                await this.prisma.questionTag.upsert({
                    where: { questionId_tagId: { questionId: link.questionId, tagId: targetId } },
