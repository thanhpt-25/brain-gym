...
import {
    Difficulty, LlmProvider, GenerationJobStatus, QualityTier,
    QuestionStatus, QuestionType,
} from '@prisma/client';
...
export class AiQuestionBankService {
    ...
    async saveLlmConfig(userId: string, dto: ConfigureLlmDto) {
        ...
        return this.prisma.userLlmConfig.upsert({
            ...
            update: { encryptedKey: encrypted, modelId: dto.modelId || null, isActive: true },
            create: { userId, provider: dto.provider, encryptedKey: encrypted, modelId: dto.modelId || null },
            select: { id: true, provider: true, modelId: true, isActive: true, createdAt: true },
        });
    }
    ...
    async getLlmConfigs(userId: string) {
        const configs = await this.prisma.userLlmConfig.findMany({
            ...
            select: { id: true, provider: true, modelId: true, isActive: true, encryptedKey: true, createdAt: true },
        });
        return configs.map((c: typeof configs[number]) => ({
            ...c,
            maskedKey: this.encryption.maskKey(this.encryption.decrypt(c.encryptedKey)),
    ...
    async validateLlmConfig(userId: string, provider: LlmProvider) {
        ...
        const llm = createLlmProvider(provider, apiKey, config.modelId || undefined);
    ...
    async estimateTokens(userId: string, dto: GenerateQuestionsDto) {
        ...
        const apiKey = this.encryption.decrypt(config.encryptedKey);
        const llm = createLlmProvider(dto.provider, apiKey, config.modelId || undefined);

        const cert = await this.prisma.certification.findFirst({ where: { id: dto.certificationId } });
        const domain = dto.domainId ? await this.prisma.domain.findUnique({ where: { id: dto.domainId } }) : null;
        const sourceChunks = dto.materialId ? await this.ingestion.getChunksForMaterial(dto.materialId) : [];
        ...
        return {
            ...estimate,
            totalEstimatedTokens: estimate.estimatedPromptTokens + estimate.estimatedCompletionTokens,
        };
    }
    ...
    async generateQuestions(userId: string, dto: GenerateQuestionsDto) {
        ...
        try {
            ...
            const rawQuestions = this.parseGeneratorResponse(genResult.content, dto.questionCount);
            ...
            try {
                ...
                scores = this.parseCriticResponse(criticResult.content, rawQuestions.length);
            } catch {
                // Critic pass failure is non-fatal — use confidence_hint fallback
                scores = rawQuestions.map(q =>
                    q.confidence_hint === 'high' ? 0.87 :
                    q.confidence_hint === 'medium' ? 0.70 : 0.50
                );
            }

            // Map to preview objects
            const previews = rawQuestions.map((q, i) => this.mapToPreview(q, scores[i], dto));
            ...
            return { jobId: job.id, questions: previews, tokenUsage: { prompt: genResult.promptTokens, completion: genResult.completionTokens } };
        } catch (err) {
            await this.prisma.questionGenerationJob.update({
                where: { id: job.id },
                data: { status: GenerationJobStatus.FAILED, errorMessage: (err as Error).message },
            });
            throw new UnprocessableEntityException(`Generation failed: ${(err as Error).message}`);
        }
    }
    ...
    async saveGeneratedQuestions(userId: string, dto: SaveGeneratedQuestionsDto) {
        ...
        for (const [index, q] of dto.questions.entries()) {
            ...
            const status = tier === QualityTier.HIGH ? QuestionStatus.APPROVED : QuestionStatus.PENDING;

            const question = await this.questions.create(userId, {
                title: q.title,
                ...
                choices: q.choices.map((c, i) => ({ label: String.fromCharCode(65 + i), content: c.content, isCorrect: c.isCorrect })),
                tags: q.tags,
                isScenario: q.isScenario,
                isTrapQuestion: q.isTrapQuestion,
            }, status, dto.jobId, tier, q.sourceChunkId);
    ...
    async mcpIntake(userId: string, dto: McpIntakeDto) {
        ...
        for (const [index, q] of dto.questions.entries()) {
            const score = q.quality_score ?? 0.70;
            ...
            const question = await this.questions.create(userId, {
                ...
                domainId: dto.domainId,
                choices: q.choices,
            }, status, undefined, tier);

            saved.push(question.id);
        }

        return { saved: saved.length, discarded: discarded.length, questionIds: saved };
    }
    ...
    private async requireLlmConfig(userId: string, provider: LlmProvider) {
        ...
        if (!config) throw new BadRequestException(`No API key configured for ${provider}. Please add one in Settings.`);
        if (!config.isActive) throw new BadRequestException(`${provider} API key is disabled.`);
        return config;
    }

    private parseGeneratorResponse(content: string, expectedCount: number): RawGeneratedQuestion[] {
        let parsed: any;
        try {
        ...
        if (!parsed?.questions || !Array.isArray(parsed.questions)) {
            throw new Error('LLM returned an invalid response format (missing "questions" array)');
        }
    ...
    private parseCriticResponse(content: string, count: number): number[] {
        ...
        if (!parsed?.results || !Array.isArray(parsed.results)) {
            return Array(count).fill(0.70);
        }
        const scores = Array(count).fill(0.70);
    ...
    private mapToPreview(raw: RawGeneratedQuestion, score: number, dto: GenerateQuestionsDto): GeneratedQuestion & { qualityScore: number; qualityTier: QualityTier | null } {
        const tier = this.scoreTotier(score);
        const correctLetters = raw.correct_answer.split(',').map(s => s.trim());
        const qType = dto.questionType || (correctLetters.length > 1 ? QuestionType.MULTIPLE : QuestionType.SINGLE);
