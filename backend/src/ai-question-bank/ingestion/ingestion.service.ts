...
export class IngestionService {
    ...
    async uploadMaterial(userId: string, dto: UploadMaterialDto, fileBuffer?: Buffer) {
        let rawText: string;

        if (dto.contentType === MaterialContentType.TEXT) {
            if (!dto.textContent) throw new BadRequestException('textContent is required for TEXT type');
            if (dto.textContent.length > MAX_TEXT_LENGTH) {
                throw new BadRequestException(`Text content exceeds maximum length of ${MAX_TEXT_LENGTH} characters`);
            }
            rawText = dto.textContent;
        } else if (dto.contentType === MaterialContentType.URL) {
            if (!dto.sourceUrl) throw new BadRequestException('sourceUrl is required for URL type');
            rawText = await this.fetchUrl(dto.sourceUrl);
        } else if (dto.contentType === MaterialContentType.PDF) {
            if (!fileBuffer) throw new BadRequestException('A PDF file is required for PDF type');
    ...
    async deleteMaterial(userId: string, materialId: string) {
        const material = await this.prisma.sourceMaterial.findUnique({ where: { id: materialId } });
        if (!material) throw new NotFoundException('Material not found');
        if (material.userId !== userId) throw new ForbiddenException();
    ...
    private async fetchUrl(url: string): Promise<string> {
        ...
        if (!response.ok) throw new BadRequestException(`Failed to fetch URL: ${response.statusText}`);
    ...
    private async parsePdf(buffer: Buffer): Promise<string> {
        ...
        if (!pdfParse) {
            throw new BadRequestException('PDF parsing is not available. Please use TEXT type instead.');
        }
