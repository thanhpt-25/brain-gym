...
describe('CertificationsService', () => {
  ...
  describe('update', () => {
    it('should throw ConflictException if new code is already used by another cert', async () => {
      ...
      await expect(service.update('uuid-1', { code: 'T-USED' }))
        .rejects.toThrow(ConflictException);
    });
    ...
    it('should delete existing domains if domains list is provided', async () => {
      ...
      expect(prisma.certification.update).toHaveBeenCalledWith(expect.objectContaining({
  ...
  describe('softDelete', () => {
    it('should set isActive to false', async () => {
      prisma.certification.findUnique.mockResolvedValue(mockCertification);
      prisma.certification.update.mockResolvedValue({ ...mockCertification, isActive: false });
