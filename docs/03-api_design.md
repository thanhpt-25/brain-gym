...
| POST | `/attempts/:id/answer` | JWT | Save or update a single answer during an attempt. `409` if the answer was already checked (interactive mode). |
| POST | `/attempts/:id/check` | JWT | Interactive mode only: grade one question, lock it and return `{ isCorrect, selectedChoiceIds, correctChoiceIds, explanation, checkedAt }`. `403` unless the caller owns an `INTERACTIVE` attempt; `400` if the question is not in the exam, the choices are invalid or the attempt is submitted; `409` if already checked. See `docs/specs/exam-interactive-mode-srs.md`. |
| POST | `/attempts/:id/submit` | JWT | Submit an attempt; calculates score and returns results. Answers checked in interactive mode are graded from the stored answer, not the payload. |
