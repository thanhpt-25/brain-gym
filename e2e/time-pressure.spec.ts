...
test.describe("Time Pressure Exam Mode", () => {
  // Requires a logged-in session and backend exam data. Skip when no E2E
  // credentials are configured (CI without secrets / no running backend).
  test.skip(
    !process.env.E2E_USER_EMAIL || !process.env.E2E_USER_PASSWORD,
    "requires E2E_USER_EMAIL / E2E_USER_PASSWORD and a running backend",
  );

  test("should display timer warnings and support Accelerated mode", async ({
    page,
  }) => {
    ...
    await page.route(
      "**/api/v1/questions?certId=aws-saa-c03*",
      async (route) => {
        await route.fulfill({
          status: 200,
          json: { meta: { total: 50 }, data: [] },
        });
      },
    );
          ...
          questions: [
            {
              id: "q1",
              title: "Q1",
              choices: [{ id: "c1", label: "a", content: "c1" }],
            },
          ],
        },
      });
    ...
    await expect(liveRegion).toHaveText(/25% time remaining/, {
      timeout: 10000,
    });
